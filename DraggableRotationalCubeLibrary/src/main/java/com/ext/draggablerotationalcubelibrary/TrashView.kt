 ...
 */
internal class TrashView(context: Context) : FrameLayout(context),
    ...
    /**
     * Root View (View including background and delete icon)
     */
     ...
     * ActionTrashIconの幅
     */
    private var mActionTrashIconBaseWidth = 0

    /**
     * ActionTrashIconの高さ
     */
    private var mActionTrashIconBaseHeight = 0

    /**
     * ActionTrashIconの最大拡大率
     */
    private var mActionTrashIconMaxScale = 0f
    ...
    /**
     * Animation (reduction) when it goes out of the frame of the delete icon
     */
    ...
    init {
        ...
        val gradientDrawable = GradientDrawable(
            ...
            intArrayOf(0x00000000, 0x50000000)
        )
    ...
    fun getWindowDrawingRect(outRect: Rect) {
        ...
        val y = mRootView.height - mTrashIconRootView.y - iconPaddingTop - iconHeight
    ...
    fun updateActionTrashIcon(width: Float, height: Float, shape: Float) {
        ...
        mActionTrashIconMaxScale = max(newWidthScale.toDouble(), newHeightScale.toDouble())
            .toFloat()
        // ENTERアニメーション作成
        mEnterScaleAnimator = ObjectAnimator.ofPropertyValuesHolder(
            mActionTrashIconView, PropertyValuesHolder.ofFloat(
                SCALE_X, mActionTrashIconMaxScale
            ), PropertyValuesHolder.ofFloat(SCALE_Y, mActionTrashIconMaxScale)
        )
        mEnterScaleAnimator!!.interpolator = OvershootInterpolator()
        mEnterScaleAnimator!!.setDuration(TRASH_ICON_SCALE_DURATION_MILLIS)
        // Exitアニメーション作成
        mExitScaleAnimator = ObjectAnimator.ofPropertyValuesHolder(
            mActionTrashIconView, PropertyValuesHolder.ofFloat(
                SCALE_X, 1.0f
            ), PropertyValuesHolder.ofFloat(SCALE_Y, 1.0f)
        )
    ...
    val trashIconCenterY: Float
        ...
        get() {
            ...
            val y = mRootView.height - mTrashIconRootView.y - iconViewHeight + iconViewPaddingBottom
    ...
    fun onTouchFloatingView(event: MotionEvent, x: Float, y: Float) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            // 長押し処理待ち
            mAnimationHandler.removeMessages(ANIMATION_CLOSE)
            mAnimationHandler.sendAnimationMessageDelayed(
                ANIMATION_OPEN,
    ...
    internal class AnimationHandler(trashView: TrashView) : Handler() {
         ...
         * アニメーションを始めた時点のアルファ値
         */
        private var mStartAlpha = 0f

        /**
         * アニメーションを始めた時点のTransitionY
         */
        private var mStartTransitionY = 0f
         ...
         * 追従対象の幅
         */
        var mTargetWidth: Float = 0f

        /**
         * 追従対象の高さ
         */
        var mTargetHeight: Float = 0f
         ...
         * Y軸の追従の範囲
         */
        private var mMoveStickyYRange = 0f
         ...
         * TrashView
         */
        private val mTrashView = WeakReference(trashView)
        ...
        override fun handleMessage(msg: Message) {
            ...
            // Initialization when the animation starts
            ...
            if (animationCode == ANIMATION_OPEN) {
                ...
                if (currentAlpha < MAX_ALPHA) {
                    val alphaTimeRate = min(
                        (elapsedTime / BACKGROUND_DURATION_MILLIS).toDouble(),
                        1.0
                    ).toFloat()
                    val alpha = min(
                        (mStartAlpha + alphaTimeRate).toDouble(),
                        MAX_ALPHA.toDouble()
                    ).toFloat()
                ...
                if (elapsedTime >= TRASH_OPEN_START_DELAY_MILLIS) {
                    ...
                    val positionX =
                        trashViewX + (mTargetPositionX + mTargetWidth) / (screenWidth + mTargetWidth) * mTrashIconLimitPosition.width() + mTrashIconLimitPosition.left
                    ...
                    val targetPositionYRate = min(
                        (2 * (mTargetPositionY + mTargetHeight) / (screenHeight + mTargetHeight)).toDouble(),
                        1.0
                    ).toFloat()
                    val stickyPositionY =
                        mMoveStickyYRange * targetPositionYRate + mTrashIconLimitPosition.height() - mMoveStickyYRange
                    val translationYTimeRate = min(
                        ((elapsedTime - TRASH_OPEN_START_DELAY_MILLIS) / TRASH_OPEN_DURATION_MILLIS).toDouble(),
                        1.0
                    ).toFloat()
                    val positionY =
                        mTrashIconLimitPosition.bottom - stickyPositionY * mOvershootInterpolator.getInterpolation(
                            translationYTimeRate
                        )
                ...
                val alphaElapseTimeRate = min(
                    (elapsedTime / BACKGROUND_DURATION_MILLIS).toDouble(),
                    1.0
                ).toFloat()
                val alpha = max(
                    (mStartAlpha - alphaElapseTimeRate).toDouble(),
                    MIN_ALPHA.toDouble()
                ).toFloat()
                backgroundView.alpha = alpha

                // Delete icon Y coordinate animation
                val translationYTimeRate = min(
                    (elapsedTime / TRASH_CLOSE_DURATION_MILLIS).toDouble(),
                    1.0
                ).toFloat()
                // If the animation has not reached the end
                if (alphaElapseTimeRate < 1.0f || translationYTimeRate < 1.0f) {
                    val position =
                        mStartTransitionY + mTrashIconLimitPosition.height() * translationYTimeRate
        ...
        fun onUpdateViewLayout() {
            ...
            val top =
                ((trashIconHeight - backgroundHeight) / 2 - TRASH_MOVE_LIMIT_TOP_OFFSET * density).toInt()
            val right = offsetX.toInt()
            val bottom = trashIconHeight
            mTrashIconLimitPosition[left, top, right] = bottom
    ...
    companion object {
         ...
         * アニメーションなしの状態を表す定数
         */
        const val ANIMATION_NONE: Int = 0

        /**
         * 背景・削除アイコンなどを表示するアニメーションを表す定数<br></br>
         * FloatingViewの追尾も含みます。
         */
        const val ANIMATION_OPEN: Int = 1

        /**
         * 背景・削除アイコンなどを消すアニメーションを表す定数
         */
        const val ANIMATION_CLOSE: Int = 2

        /**
         * 背景・削除アイコンなどを即時に消すことを表す定数
         */
        const val ANIMATION_FORCE_CLOSE: Int = 3
